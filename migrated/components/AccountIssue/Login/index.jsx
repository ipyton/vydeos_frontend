...
const NetworkError = dynamic(() => import('../../Errors/NetworkError'));
...
export default function Login(props) {
  ...
  const [AccountUtil, setAccountUtil] = useState(null);
  const [AuthUtil, setAuthUtil] = useState(null);

  // Load utilities dynamically
  useEffect(() => {
    const loadUtils = async () => {
      if (typeof window !== 'undefined') {
        const accountUtilModule = await import('../../../utils/AccountUtil');
        const authUtilModule = await import('../../../utils/AuthUtil');
        setAccountUtil(accountUtilModule.default);
        setAuthUtil(authUtilModule.default);
      }
    };
    loadUtils();
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!validate() || !AccountUtil || !AuthUtil) {
      return;
    }
              ...
              (response1) => {
                if (response1.data.code === 0) {
                  localforage.setItem("paths", JSON.parse(response1.data.message)).then(() => {
                    setTimeout(() => {
                      // Use the login props to update auth state
                      props.setLogin(true);
